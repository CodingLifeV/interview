...
2. 传输层：用寻址机制来标识一个特定的应用程序端口号，封装一个报文头，包含了源端口和目的端口信息
3. 网络层：对传输层中处理完的数据再次封装，使用 IP 协议添加上 32 位的源 IP 地址和目的 IP 地址封装好 IP 数据报，
   使用 ARP 协议把 IP 地址解析成物理地址。
...
1. 客户端发起 HTTPS 请求，然后连接到 server 的 443 端口
